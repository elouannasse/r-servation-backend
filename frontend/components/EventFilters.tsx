...
export default function EventFilters({ onFilterChange, events }: any) {
  ...
  const applyFilters = () => {
    ...
    if (dateStart) {
      filtered = filtered.filter((e: any) => new Date(e.date) >= new Date(dateStart));
    }

    if (dateEnd) {
      filtered = filtered.filter((e: any) => new Date(e.date) <= new Date(dateEnd));
    }
  ...
  const activeFiltersCount = [search, status !== 'all', location !== 'all', dateStart, dateEnd].filter(Boolean).length;
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {search && (
            <span className="inline-flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
              Recherche: {search}
              <button onClick={() => setSearch('')} className="hover:bg-blue-200 rounded-full p-0.5 transition">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {status !== 'all' && (
            <span className="inline-flex items-center gap-1 px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">
              Statut: {status}
              <button onClick={() => setStatus('all')} className="hover:bg-green-200 rounded-full p-0.5 transition">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {location !== 'all' && (
            <span className="inline-flex items-center gap-1 px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-sm font-medium">
              Lieu: {location}
              <button onClick={() => setLocation('all')} className="hover:bg-purple-200 rounded-full p-0.5 transition">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {(dateStart || dateEnd) && (
            <span className="inline-flex items-center gap-1 px-3 py-1 bg-orange-100 text-orange-800 rounded-full text-sm font-medium">
              ...
              <button onClick={() => { setDateStart(''); setDateEnd(''); }} className="hover:bg-orange-200 rounded-full p-0.5 transition">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
